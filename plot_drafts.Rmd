...
demographic_props = all_data %>%
  ...
  mutate(ORG_TYPE = 
         ...
         CAT_TYPE = ifelse(str_detect(CATEGORY, "^WLC_A[1-9]|^RC[CL]_A[1-9]"), "Age Group", 
                           ifelse(str_detect(CATEGORY, "^WLC_G[FM]|^RC[CL]_G[FM]"), "Gender", 
                                  ifelse(str_detect(CATEGORY, "^WLC_R[A-Z]_|^RC[CL]_R[A-Z]_"), "Race",
                                         ifelse(str_detect(CATEGORY, "^WLC_B[ABOU]|^RC[CL]_B[ABOU]"), "Blood Type",
                                                if_else(str_detect(CATEGORY, "^WLC_KI|^RC[CL]_.{3}_|^RC[CL]_KI"), "Primary Disease", ifelse(str_detect(CATEGORY, "[A-Z]{3}_P[RT][AX]"), "High Ab Risk",
                                                                                                                                            if_else(str_detect(CATEGORY, "RC[CL]_BMI"), "BMI", "")))))))) %>%
  filter(!is.na(CATEGORY_PROP))
  group_by(CTR_ID, ORG_TYPE, CAT_TYPE)
