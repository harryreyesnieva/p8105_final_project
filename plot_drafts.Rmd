...
age_grps = c("<2", "2-11", "12-17", "18-34", "35-49", "50-64", ">65", "65-69", ">70" )
names(age_grps) = age_pats

race_pats = c("RA", "RB", "RH", "RO", "RU", "RW")
race_grps = c("Asian", "Black", "Hispanic", "Other", "Unknown", "White")
names(race_grps) = race_pats


dz_pats = c("DIA", "GLO", "HYP", "MIS", "NEO", "OTK", "POL", "VAS", "RET", "TUB", "CON")
dz_pats = c(dz_pats, c(paste("KI",dz_pats, sep = "")))
dz_grps = c("Diabetes", "Glomerular", "Hypertension", "Missing", "Neoplasms", "Other", "Polycystic", "Vascular", "Retransplant", "Tubular/Interstial", "Congenital/Familial/Metabolic")
...
demographic_props = all_data %>%
  ...
  mutate(
    ...
    CATEGORY = if_else(CAT_TYPE == "Gender", if_else(str_sub(CATEGORY,6,6) == "M", "Male", "Female"), CATEGORY),
    CATEGORY = if_else(CAT_TYPE == "Race", 
    unlist(lapply(str_replace(str_sub(CATEGORY, 5, -1), "_[AC].*$", ""), function(x) race_grps[x])), CATEGORY),
    CATEGORY = ifelse(CAT_TYPE == "Blood Type", str_replace(str_sub(CATEGORY, 6, -1), "_[AC].*$", ""), CATEGORY),
    CATEGORY = ifelse(CAT_TYPE == "High Ab Risk", if_else(CATEGORY == "RCC_PRA80_C", "Peak PRA >= 80%", "Retransplant"), CATEGORY),
...
demographic_props %>%
  ...
  group_by(ORG_TYPE, CAT_TYPE, CATEGORY) %>%
