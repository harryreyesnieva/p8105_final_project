---
title: "Organ Transplant Dashboard"
output: 
...
organ_age = df_all_age %>% pull(org)
organ_race = df_all_demographics %>% pull(org)
organ_gender = df_all_gender %>% pull(org)

# selectInput widget
selectInput(
  "organ_race", 
  label = h3("Select demographics organ"),
  choices = organ_race, selected = "KI")

# selectInput widget
selectInput(
  "organ_age", 
  label = h3("Select age organ"),
  choices = organ_age, selected = "KI")

# selectInput widget
selectInput(
  "organ_gender", 
  label = h3("Select gender organ"),
  choices = organ_gender, selected = "KI")

labels = c("Less Than 2 Years", "2 to 11 Years", "12 to 17 Years", "18 to 34 Years", "35 to 49 Years", "50 to 64 Years", "64 to 69 Years", "More Than 70 Years")
...
renderPlotly({ 
  ...
  plot_ly(
    x = ~zipcode, y = ~race_category_percent, type = "scatter", mode = "markers",
...
renderPlotly({ 
  ...
  plot_ly(
    ...
    alpha = 0.9, color = ~age_category, text = ~labels)})
```
...
renderPlotly({ 
  ...
  plot_ly(
    x = ~zipcode, y = ~gender_category_percent, type = "scatter", mode = "markers",
