...
str(df_one_names)
df_one_dropna = drop_na(df_one_names) ##dropna error
str(df_one_dropna) 
df_one_dropna[, c(4,6:60)] <- sapply(df_one_dropna[, c(4,6:60)], as.numeric)
str(df_one_dropna)
```

```{r}
df_one_plot = df_one_dropna %>% mutate(newlistings_percent_mortality = 100*died_center_all/newlistings_center_all) %>% ggplot(aes(x=newlistings_center_all, y = newlistings_percent_mortality)) + geom_point()
```