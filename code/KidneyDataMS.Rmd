...
I will delete an extraneous row, drop missing values, and convert the appropriate columns from character to numeric.
```{r}
...
print(plot)
```
I will plot the second set of exposure variables
```{r}
x2 = df_one_merge[, c(21:35)]
plot = ggplot(gather(x2), aes(value)) + geom_histogram(bins = 100) + facet_wrap(~key, scales = "free_y") +coord_cartesian(xlim = c(0, 100)) + labs(title = "Exposure Frequency Part2", subtitle = "SRTR Kidney Transplant Data, August 2020 Release", x = "Exposure (Number)", y = "Frequency") + theme_minimal()
print(plot)
```
I will plot the third set of exposure variables
```{r}
...
print(plot)
```
I will plot the fourth set of exposure variables
```{r}
x4 = df_one_merge[, c(44:51)]
plot = ggplot(gather(x4), aes(value)) + geom_histogram(bins = 100) + facet_wrap(~key, scales = "free_y") +coord_cartesian(xlim = c(0, 200)) + labs(title = "Exposure Frequency Part4", subtitle = "SRTR Kidney Transplant Data, August 2020 Release", x = "Exposure (Number)", y = "Frequency") + theme_minimal()
print(plot)
```
...
I will plot the outcome variables to get a sense of which have the most variability.
```{r}
...
```
Now I will make a leaflet plot of kidney transplant centers in the United States.
```{r, eval = FALSE}
...
I will now import the data frame from another sheet that contains center names and zipcodes only.
```{r}
...
print(colnames)
```
I will rename and clean the columns for the patient characteristics data set.
```{r}
...
view(df_two_clean)
```
I will make a dataframe of patient demographics and plot the distributions by zipcode.
```{r}
...
```
Now I will make an age dataframe and plot the age distributions by zipcode.
```{r}
...
plot
```
Now I will make a comorbidities dataframe and plot the pra score distributions by zipcode.
```{r}
...
```
Now, I will make a blood type dataframe and plot the blood type distributions by zipcode.
```{r}
...
plot
```
Now I will merge the pra and comorbidities dataframes and make scatter plots to investigate for correlations.
```{r}
...
Now I will merge the pra and age dataframes and and make scatter plots to investigate for correlations.
```{r}
...
Now I will merge the pra and gender dataframes and make scatter plots to investigate for correlations.
```{r}
