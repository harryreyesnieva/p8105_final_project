...
#view(df_all_age)
facet_labels = c("Heart" = "HR", "Heart Lung" - "HL", "Intestine" = "IN", "Kidney" = "KI", "Kidney Pancreas" = "KP", "Liver" = "LI", "Lung" = "LU", "Pancreas" = "PA")
```
...
plot = df_all_age %>% ggplot(aes(x=zipcode, y =age_category_percent, color = age_category)) + geom_point() + 
  ...
  ) + theme_minimal()  + theme(axis.text.x=element_text(angle=90,hjust=1)) + scale_color_hue(labels = c("Less Than 2 Years", "2 to 11 Years", "12 to 17 Years", "18 to 34 Years", "35 to 49 Years", "50 to 64 Years", "64 to 69 Years", "More Than 70 Years")) + facet_wrap(~org, labeller = as_labeller(facet_labels))
print(plot)
```
I will repeat for gender
```{r}
